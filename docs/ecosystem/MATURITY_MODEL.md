...
| 6 | **Distributed** | Published as an installable package with a real version (`npm`/`PyPI`) that `skill_version`/`contract_version` can be checked against, per `VERSIONING.md`. | **Automatic as of 2026-09-06** (`dashboard/aggregator/src/packageRegistry.ts`): a live lookup against npm's/PyPI's own public registry API, keyed off `capability-status.json`'s documented `distribution.kind`/`package` (which still names *which* registry and package to check — the aggregator no longer trusts that a version exists without confirming it live). A documented claim the live lookup cannot confirm (renamed package, registry outage, ...) reports `UNKNOWN`, distinct from and never silently treated as level 6 met. |
